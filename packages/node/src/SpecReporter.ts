/* eslint-disable no-console */
import {
    IExecutionReporter,
    RunContext,
    SuiteExecutionResult,
    SuiteHandle,
    TestExecutionResult,
    TestHandle,
} from "./IExecutionReporter";

export class SpecReporter implements IExecutionReporter {
    private indents = 0;

    public onStartRun(context: RunContext): void | Promise<void> {
        console.log();
    }

    public onStartSuite(suite: SuiteHandle): void | Promise<void> {
        this.indents++;
        console.log(color("suite", "%s%s"), this.indent(), suite.name);
    }

    public onFinishSuite(suite: SuiteHandle, suiteResult: SuiteExecutionResult): void | Promise<void> {
        this.indents--;
        if (this.indents === 1) {
            console.log();
        }
    }

    public onStartTest(test: TestHandle): void | Promise<void> {
        // noop
    }

    public onFinishTest(test: TestHandle, testResult: TestExecutionResult): void | Promise<void> {
        const fmt = this.indent() + color("checkmark", "  " + symbols.ok) + color("pass", " %s");
        console.log(fmt, test.name);
    }

    public onFinishRun(context: RunContext): void | Promise<void> {
        // noop
    }

    private indent(): string {
        return Array(this.indents).join("  ");
    }
}

const symbols = {
    ok: process.platform === "win32" ? "\u221A" : "✓",
    err: process.platform === "win32" ? "\u00D7" : "✖",
    dot: "․",
    comma: ",",
    bang: "!",
};

const colors = {
    pass: 90,
    fail: 31,
    "bright pass": 92,
    "bright fail": 91,
    "bright yellow": 93,
    pending: 36,
    suite: 0,
    "error title": 0,
    "error message": 31,
    "error stack": 90,
    checkmark: 32,
    fast: 90,
    medium: 33,
    slow: 31,
    green: 32,
    light: 90,
    "diff gutter": 90,
    "diff added": 32,
    "diff removed": 31,
    "diff added inline": "30;42",
    "diff removed inline": "30;41",
};

function color(type: "suite" | "checkmark" | "pass", str: string): string {
    // return `\u001b[${colors[type]}m${str}\u001b[0m`;
    return "\u001b[" + colors[type] + "m" + str + "\u001b[0m";
}
